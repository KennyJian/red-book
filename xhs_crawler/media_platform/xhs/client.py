...
# import config  # 注释掉，使用简化版本
from ...base.base_crawler import AbstractApiClient
# from proxy.proxy_mixin import ProxyRefreshMixin  # 注释掉，简化版本不需要代理池
...
class XiaoHongShuClient(AbstractApiClient):  # 简化版本，移除 ProxyRefreshMixin
    ...
    def __init__(
        ...
        *,
        ...
        # Initialize proxy pool (from ProxyRefreshMixin) - 简化版本不需要
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_fixed(1), retry=retry_if_not_exception_type((NoteNotFoundError, CaptchaRequiredError)))
    async def request(self, method, url, **kwargs) -> Union[str, Any]:
        ...
        """
        # Check if proxy is expired before each request - 简化版本不需要
        ...
        if response.status_code == 471 or response.status_code == 461:
            verify_type = response.headers.get("Verifytype", response.headers.get("verifytype", ""))
            verify_uuid = response.headers.get("Verifyuuid", response.headers.get("verifyuuid", ""))
        ...
        try:
            ...
            utils.logger.error(f"[XiaoHongShuClient.request] response not json: {e}")
            raise DataFetchError(str(e))
        if not isinstance(data, dict):
            raise DataFetchError("response data is not dict")
        if data.get("success"):
    ...
    async def get_note_media(self, url: str) -> Union[bytes, None]:
        # Check if proxy is expired before request - 简化版本不需要
        # await self._refresh_proxy_if_expired()
    ...
    async def get_note_by_id(
        ...
        if not res or not isinstance(res, dict):
            utils.logger.warning(f"[XiaoHongShuClient.get_note_by_id] note id:{note_id} empty or non-dict response type={type(res).__name__}")
            return dict()
        items = res.get("items")
        if not items or not isinstance(items, list):
            utils.logger.warning(f"[XiaoHongShuClient.get_note_by_id] note id:{note_id} no items, res keys:{list(res.keys()) if isinstance(res, dict) else type(res)}")
            return dict()
        first = items[0] if items else None
        if not isinstance(first, dict):
            return dict()
        res_dict = first.get("note_card")
        if isinstance(res_dict, dict):
            return res_dict
        # 兼容不同返回结构：有的在 note_card 下，有的直接在 first 里
        if first.get("note_id") or first.get("title") is not None:
            return first
        utils.logger.warning(f"[XiaoHongShuClient.get_note_by_id] note id:{note_id} unexpected structure, first keys:{list(first.keys())}")
        return dict()
    ...
    async def get_all_notes_by_creator(
        ...
        notes_has_more = True
        notes_cursor = ""
        while notes_has_more and len(result) < CRAWLER_MAX_NOTES_COUNT:
            ...
            remaining = CRAWLER_MAX_NOTES_COUNT - len(result)
            if remaining <= 0:
                break
