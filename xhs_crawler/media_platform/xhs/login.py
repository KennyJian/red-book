...
# from cache.cache_factory import CacheFactory  # 简化版本不需要缓存
...
class XiaoHongShuLogin(AbstractLogin):
    ...
    def __init__(self,
        ...
        # config.LOGIN_TYPE = login_type  # 简化版本
    ...
    async def login_by_mobile(self):
        ...
        # cache_client = CacheFactory.create_cache(config.CACHE_TYPE_MEMORY)  # 简化版本
        max_get_sms_code_time = 60 * 2  # Maximum time to get verification code is 2 minutes
        no_logged_in_session = ""
        # 简化版本：直接从用户输入获取验证码，或使用环境变量
        import os
        sms_code_key = f"xhs_{self.login_phone}"
        sms_code_value = os.environ.get(sms_code_key)  # 从环境变量获取
        while max_get_sms_code_time > 0:
            utils.logger.info(f"[XiaoHongShuLogin.login_by_mobile] get sms code remaining time {max_get_sms_code_time}s ...")
            await asyncio.sleep(1)
            ...
            await sms_code_input_ele.fill(value=str(sms_code_value))  # Enter SMS verification code
            await asyncio.sleep(0.5)
